...
G4Material* BuildHydrogenGasMaterial(G4Element* hydrogen,
                                     G4double pressure,
                                     G4double temperature,
                                     G4double density)
{
  auto* material =
    new G4Material("FFRE_H2Gas", density, 1, kStateGas, temperature, pressure);
...
{
  if (SingleFuelParticleViewEnabled()) {
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  fFuelBedHydrogenEnabled =
    !singleFuelParticleView && ReadEnvFlag("FFRE_FILL_SHAPE1_H2", false);
  fFuelBedHydrogenTemperature =
    std::max(1., ReadEnvDouble("FFRE_H2_TEMPERATURE_K", 300.))*kelvin;
  fFuelBedHydrogenPressure =
    std::max(0., ReadEnvDouble("FFRE_H2_PRESSURE_ATM", 1.))*atmosphere;

  const G4double h2DensityOverride =
    ReadEnvDouble("FFRE_H2_DENSITY_KG_M3", -1.);
  if (h2DensityOverride > 0.) {
    fFuelBedHydrogenDensity = h2DensityOverride*kg/(m*m*m);
  } else {
    constexpr G4double h2MolarMassKgPerMol = 2.01588e-3;
    constexpr G4double gasConstantJPerMolK = 8.31446261815324;
    const G4double pressurePa =
      (fFuelBedHydrogenPressure/atmosphere)*101325.;
    const G4double temperatureK = fFuelBedHydrogenTemperature/kelvin;
    const G4double densityKgPerM3 =
      pressurePa*h2MolarMassKgPerMol/(gasConstantJPerMolK*temperatureK);
    fFuelBedHydrogenDensity = densityKgPerM3*kg/(m*m*m);
  }

  G4Material* h2_mat = nullptr;
  if (fFuelBedHydrogenEnabled) {
    h2_mat = BuildHydrogenGasMaterial(elH,
                                      fFuelBedHydrogenPressure,
                                      fFuelBedHydrogenTemperature,
                                      fFuelBedHydrogenDensity);

    G4cout
      << "[FuelBedH2] Shape1 startup fill enabled: pressure = "
      << fFuelBedHydrogenPressure/atmosphere
      << " atm, temperature = "
      << fFuelBedHydrogenTemperature/kelvin
      << " K, density = "
      << fFuelBedHydrogenDensity/(kg/(m*m*m))
      << " kg/m3"
      << G4endl;
  }
  ...
  G4Material* shape1_mat =
    fFuelBedHydrogenEnabled ? h2_mat : env_mat;
    ...
    for (G4int iz = 0; iz < paperAxialBallCount; ++iz) {
      ...
      for (G4int iy = 0; iy < paperRadialBallCountY; ++iy) {
        ...
        for (G4int ix = 0; ix < paperRadialBallCountX; ++ix) {
          ...
          new G4PVPlacement(nullptr, particlePos, logicFuelShell, "FuelShell",
                            logicShape1, false, particleCopyNo++, false);
          fFuelShellCenters.push_back(particleWorldPos);
